  ...
  return (
    <div className="animate-fade-in home-shell">
      <section className="home-hero">
        <div>
          <div className="home-kicker"><Sparkles size={16} /> Smart exam training</div>
          <h1 style={{ marginBottom: '0.45rem' }}>Learn faster, battle smarter</h1>
          <p className="text-muted" style={{ maxWidth: '60ch' }}>
            Track your progress every day, jump into random exams, and climb battle rankings with a smoother practice workflow.
          </p>
          <div className="flex gap-3 mt-6" style={{ flexWrap: 'wrap' }}>
            ...
            <button className="btn btn-secondary" style={{ borderRadius: '999px' }} onClick={() => navigate('/battle/join')}>
              <Swords size={16} /> Join battle
            </button>
          </div>
        </div>
        <div className="home-hero-badge">
          <Trophy size={18} /> Keep your streak alive
        </div>
      </section>

      <section className="home-stats-grid">
        <article className="card home-stat-card">
          <div className="home-stat-label">Total attempts</div>
          <div className="home-stat-value">{loading ? '...' : stats.total}</div>
        </article>
        <article className="card home-stat-card">
          <div className="home-stat-label">Average score</div>
        ...
        <article className="card home-stat-card">
          ...
          <div className="home-stat-value"><Flame size={18} /> {loading ? '...' : stats.streak} day</div>
        </article>
      </section>
      ...
      <section className="card home-goal-card">
        <div className="flex justify-between items-center" style={{ gap: '0.8rem', flexWrap: 'wrap' }}>
          <div>
            <div className="home-stat-label"><CalendarCheck2 size={16} /> Daily goal</div>
            <div className="text-sm text-muted">Today's completed attempts: {stats.todayCount}</div>
          </div>
          <div className="flex items-center gap-2">
            <label htmlFor="daily-goal" className="text-sm text-muted">Target:</label>
            <input
              id="daily-goal"
              type="number"
              min={1}
